...
class DBCharacterEvidence {
	...
	public static function get($sourceRef, $version = '4.0') {
		$q = Env::$db->prepare('SELECT * FROM "character_evidences" WHERE "source_identifier" = ? AND "version" <= ? ORDER BY "version" DESC LIMIT 1');
	...
	public static function add($source_identifier, $image, $data, $version) {
		$q = Env::$db->prepare('INSERT INTO "character_evidences" (source_identifier, data, version) VALUES(?, ?, ?, ?)');
